...
Multiplayer Wikipedia racing for people on the same network. Everyone gets the
same start and target article; first one there by clicking links only wins.

There is **no server**. Each player runs the same script, the copies find each
other over the LAN, and they gossip directly. Nobody has to host, and nobody
closing their laptop ends the game.
...
Either way, type your name in the sidebar, and when everyone shows up hit
**Set up a race**. Everyone gets a 3-2-1 countdown together.
...
```
--host                print a link others can play from without installing
